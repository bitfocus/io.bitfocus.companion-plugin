...
$SD.on('connected', (jsn) => {
	...
	const selector = document.getElementById('buttonselector');
	for (let y = 1; y <= 4; ++y) {
		for (let x = 1; x <= 8; ++x) {
			const newobj = document.createElement('option');
			const xvalue = x + ((y-1) * 8);
			newobj.value = x + ':' + y;
			newobj.append(document.createTextNode("Button x." + xvalue));
			selector.append(newobj);
		}
	}
	...
	for (let i = 1; i <= 99; ++i) {
		const newobj = document.createElement('option');
		newobj.value = i;
		newobj.append(document.createTextNode(i));
		pageselect.append(newobj);
	}

	getConnectionStatus(jsn);
	...
	if (settings && Object.keys(settings).length > 0) {
		...
		const currentButton = (jsn.actionInfo.payload.coordinates.column + 1) + ':' + (jsn.actionInfo.payload.coordinates.row + 1);
		saveSettings({
			key: 'buttonselector',
			value: currentButton
		});
...
$SD.on('setImage', jsn => {
	console.log("Setimage: ", jsn);
});

$SD.on('sendToPropertyInspector', jsn => {
	const pl = jsn.payload;
	console.log("SENDTOPROPETYINSPECT: ", jsn);
    /**
	...
	} else if (pl.hasOwnProperty('connection')) {
		const el = document.getElementById('companion_connect');
		el.innerHTML = '<summary>' + pl.connection + '</summary>';
		el.className = 'message ' + pl.class;
		el.style = '';

		if (pl.version && pl.version === 2) {
			document.getElementById('show_pageselector').style = 'display: flex';
		}
	} else if (pl.hasOwnProperty('settings')) {
		console.log("New settings from sever");
...
function getConnectionStatus(jsn) {
	var payload = {
		command: 'get_connection',
		values: {}
	};
	$SD.api.sendToPlugin(jsn.uuid, jsn.actionInfo.action, payload);
}
